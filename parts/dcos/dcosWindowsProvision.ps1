...
# First up, download the runasxbox util
    curl.exe -fLsS --retry 20  -o c:\AzureData\runasxbox.exe https://dcosdevstorage.blob.core.windows.net/tmp/RunAsXbox.exe
...
'@  
  ...
  #  $adminUser = "dcos-service"   # Overwriting the arg
    ...
    & curl.exe -fLsS --retry 20 $dcosInstallUrl -o $global:BootstrapInstallDir\dcos_install.ps1
    if ($LASTEXITCODE -ne 0) {
