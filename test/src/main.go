...
const usage = `Usage:
  dcos-engine-test <options>

  Options:
	-c <configuration.json> : JSON file containing a list of deployment configurations.
		Refer to dcos-engine/test/config/dcos1.11-lnx.json for examples
	-d <dcos-engine executable path>
...
func (m *TestManager) testRun(d config.Deployment, index, attempt int, timeout time.Duration) *report.ErrorInfo {
	...
	instanceName := fmt.Sprintf("dcos%d", rand.Intn(0x0ffffff))
...
func mainInternal() error {
	var configFile, logErrorFile, dcosExePath string
	var err error

	flag.StringVar(&configFile, "c", "", "deployment configurations")
	flag.StringVar(&dcosExePath, "d", "", "dcos-engine exec path")
	flag.StringVar(&logErrorFile, "e", "dcos-engine-errors.json", "logError config file")
	flag.Usage = func() {
		fmt.Println(usage)
	}
	flag.Parse()
	...
	// get ace-engine exec
	if dcosExePath == "" {
		return fmt.Errorf("dcos-engine exec path is not provided")
	}
	if _, err = os.Stat(dcosExePath); err != nil {
		return err
	}
	os.Setenv("DCOS_ENGINE_EXE", dcosExePath)
