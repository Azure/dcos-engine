...
import (
	...
	"github.com/Azure/dcos-engine/test/dcos-engine-test/config"
	"github.com/Azure/dcos-engine/test/dcos-engine-test/metrics"
	"github.com/Azure/dcos-engine/test/dcos-engine-test/promote"
	"github.com/Azure/dcos-engine/test/dcos-engine-test/report"
)
...
const usage = `Usage:
  ...
  Options:
    -c <configuration.json> : JSON file containing a list of deployment configurations.
		Refer to dcos-engine/test/dcos-engine-test/dcos-engine-test.json for examples
	-d <dcos-engine root directory>
...
func mainInternal() error {
	...
	flag.StringVar(&rootDir, "d", "", "dcos-engine root directory")
	...
	if rootDir == "" {
		return fmt.Errorf("dcos-engine root directory is not provided")
	}
